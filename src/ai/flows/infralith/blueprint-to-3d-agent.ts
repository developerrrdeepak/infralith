...
const extractDimensionPairFromText = (value: string): { widthM: number; depthM: number } | null => {
  ...
  if (!text) return null;
  // Normalize OCR separators so `3.5 x 4.2`, `3.5×4.2`, `3.5 by 4.2`, and `3.5*4.2` are parsed consistently.
...
const normalizeFurniturePlacement = (
  ...
  _walls: GeometricReconstruction['walls'],
  _rooms: GeometricReconstruction['rooms'],
  _doors: GeometricReconstruction['doors'],
  _windows: GeometricReconstruction['windows']
): GeometricReconstruction['furnitures'] => {
  return (furnitures || [])
    .map((item) => {
      const position = asPoint2D(item?.position);
      const width = toFiniteScalar(item?.width);
      const depth = toFiniteScalar(item?.depth);
      const height = toFiniteScalar(item?.height);
      if (!position || width == null || depth == null || height == null) return null;
      ...
      return {
        ...
        floor_level: toFloorBucket(item?.floor_level),
      };
    })
...
const normalizeReconstructionGeometry = (payload: GeometricReconstruction): GeometricReconstruction => {
  ...
  for (const wall of payload.walls) {
    ...
    const evidenceHeight = Number.isFinite(parsedHeight) && parsedHeight >= 2.0
      ? parsedHeight
      : observedWallHeight;
    if (!Number.isFinite(Number(evidenceThickness)) || !Number.isFinite(Number(evidenceHeight))) {
    ...
    normalizedWalls.push({
      ...
      thickness: Number(Number(evidenceThickness).toFixed(3)),
      height: Number(Number(evidenceHeight).toFixed(3)),
    });
  }
  ...
  for (const door of payload.doors || []) {
    ...
    const evidenceHeight = Number.isFinite(parsedHeight) && parsedHeight >= 1.75
      ? parsedHeight
      : observedDoorHeight;
    if (!Number.isFinite(Number(evidenceWidth)) || !Number.isFinite(Number(evidenceHeight))) {
      droppedDoors += 1;
      continue;
    }
    const width = Number(evidenceWidth);
    const height = Number(evidenceHeight);
  ...
  for (const win of payload.windows || []) {
    ...
    const evidenceSill = Number.isFinite(parsedSill) && parsedSill >= 0.35
      ? parsedSill
      : observedWindowSill;
    if (!Number.isFinite(Number(evidenceWidth)) || !Number.isFinite(Number(evidenceSill))) {
      droppedWindows += 1;
      continue;
    }
    const width = Number(evidenceWidth);
    const sillHeight = Number(evidenceSill);
  ...
  const resolvedRooms = normalizedRooms;
  ...
  if (droppedWallsForMissingGeometry > 0) {
    normalized.conflicts = [
      ...(normalized.conflicts || []),
      {
        type: 'structural',
        severity: 'medium',
        description: `Dropped ${droppedWallsForMissingGeometry} wall segment(s) because thickness/height evidence was unavailable in the blueprint output.`,
...
export async function processBlueprintTo3D(imageUrl: string): Promise<GeometricReconstruction> {
  ...
  let lineRecords: BlueprintLineRecord[] = [];
  try {
    ...
    lineRecords = [];
    traceLog('Infralith Vision Engine', traceId, '1/9', 'failed to resolve persisted line semantics DB; continuing without static line semantics', {
