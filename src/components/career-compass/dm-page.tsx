...
export default function DMPage() {
  ...
  const handleSearchChange = (value: string) => {
    ...
    lookupDebounce.current = setTimeout(async () => {
      ...
      try {
        let resolvedUser = await userDbService.getUserByEmail(normalized);

        // Fallback 1: directory search endpoint for cases where exact-email lookup
        // is stale but search index already has the user.
        if (!resolvedUser) {
          try {
            const candidates = await userDbService.searchUsers(normalized);
            const exact = candidates.find((u) => normalizeEmail(u.email || '') === normalized);
            if (exact) resolvedUser = exact;
          } catch (error) {
            console.warn('Directory search fallback failed', error);
          }
        }

        // Fallback 2: in-memory directory snapshot for instant UX.
        const cachedMatch =
          resolvedUser ||
